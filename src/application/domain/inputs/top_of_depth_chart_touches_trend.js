...
function TopOfDepthChartTouchesTrend(gamesToConsider) {
    RecentTrend.call(this, gamesToConsider);
}
TopOfDepthChartTouchesTrend.prototype = _.create(RecentTrend.prototype, { constructor: TopOfDepthChartTouchesTrend });

TopOfDepthChartTouchesTrend.prototype.getTrendValueForPlayerAndGame = function getTrendValueForPlayerAndGame(player, game) {
    return teamRepository.findOneByName(player.team)
        .then(function calculateDepthChartTrend(team) {
            ...
            return q.all(teamPlayerPromises)
                .then(function sumTouches(teamPlayers) {
                    return teamPlayers.reduce(function sumLowerDepthTouches(total, teamPlayer) {
                        ...
                        if(!teamPlayerGame) {
                            logger.warn('Missing game for player "' + teamPlayer.name + '" in week ' + game.week + ' of ' + game.year + ' when calculating TopOfDepthChartTouchesTrend');
                            return 0;
                        }

                        return total + teamPlayerGame.stats.touches;
                    }, 0);
                });
        });
};

module.exports = TopOfDepthChartTouchesTrend;
